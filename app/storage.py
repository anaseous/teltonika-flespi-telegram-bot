...
'''
    with db() as con: con.executescript(sql)
def set_config(k,v):
...
def upsert_state(device_id, **kw):
    ...
    cols=['device_id','last_ts','last_lat','last_lon','last_speed','last_ignition','last_location_sent_ts','trip_open','trip_start_ts','idle_start_ts','last_towing_ts']
    vals=[cur.get(c) for c in cols]
